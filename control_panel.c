...
static uint32_t ulCurrentUsers = 0;
...
void feedback_full(void);
void feedback_empty(void);
...
int main()
{
    ...
    // gpio_set_irq_enabled_with_callback(BUTTON_B, GPIO_IRQ_EDGE_FALL, true, &gpio_irq_hanlder);
...
void feedback_update(void) {
    ...
    ssd1306_draw_string(&ssd, user, 2, 2);

    char vacancies[15];
    sprintf(vacancies, "Vagas: %d", remaining_vacancies);
    ssd1306_draw_string(&ssd, vacancies, 2, 20);
    ssd1306_send_data(&ssd);
...
void feedback_full(void) {
    ...
    char user[15];
    sprintf(user, "Usuarios: %d", ulCurrentUsers);
    ssd1306_draw_string(&ssd, user, 2, 0);
    ssd1306_draw_string(&ssd, "LIMITE CHEIO", 0, 30);
    ssd1306_send_data(&ssd);
...
void feedback_empty(void) {
    ...
    sprintf(user, "Usuarios: %d", ulCurrentUsers);
    ssd1306_draw_string(&ssd, user, 2, 0);
    ssd1306_draw_string(&ssd, "LIMITE VAZIO", 0, 30);
    ssd1306_send_data(&ssd);
